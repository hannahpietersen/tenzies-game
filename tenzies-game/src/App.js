...
function App() {
  ...
  //initialize state to hold rolls stats
  ...
  // Setting bestRolls to localStorage every item bestRolls changes
  React.useEffect(() => {
    localStorage.setItem("bestRolls", JSON.stringify(bestRolls));
  }, [bestRolls]);

  // Setting bestTime to localStorage every item bestTime changes
  React.useEffect(() => {
      ...
      <main>
        ...
        {!tenzies && (
          <p className="instructions">
            Roll until all numbers are the same.
            <br /> Click each number to freeze it at its current value between
            rolls.
          </p>
        )}
        ...
        <div className="stats-container">
          <p>Rolls: {rolls}</p>
          <p>
            {/* divide the time by 10 because millisecond
            ...
            slice and pass in a parameter of -2 so that when the 
